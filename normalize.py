...
def prepare_db():
    ...
    #create tables
    cursor_mysql.execute('create table engines(engine_model varchar(255),engine_power integer(255),engine_volume integer(255), engine_type varchar(255), PRIMARY KEY(engine_model))')
    cursor_mysql.execute('create table transmissions(transmission_model varchar(255),transmission_type varchar(255),transmission_gears_number integer(255), PRIMARY KEY(transmission_model))')
    cursor_mysql.execute('create table brands(brand_name varchar(255), brand_creator_country varchar(255), PRIMARY KEY(brand_name))')
    cursor_mysql.execute('create table wheels(wheel_model varchar(255), wheel_radius integer(255), wheel_color varchar(255), PRIMARY KEY(wheel_model))')
    cursor_mysql.execute('create table cars(model varchar(255), brand varchar(255),engine varchar(255), transmission varchar(255), price integer(255),wheel varchar(255), PRIMARY KEY(model), FOREIGN KEY (brand) REFERENCES brands(brand_name), FOREIGN KEY (engine) REFERENCES engines(engine_model), FOREIGN KEY (transmission) REFERENCES transmissions(transmission_model), FOREIGN KEY (wheel) REFERENCES wheels(wheel_model))')
    conn_mysql.commit()
    conn_mysql.close()
